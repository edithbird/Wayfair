...
```{r}
library(rvest)
pageZone <- read_html("https://state.1keydata.com/time-zone-state.php")
...
```{r}
ggplot(fraud, aes(x = OrderDate))+ geom_bar() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
```

Below is the same data grouped by day of the week. 

```{r}
fraud$OrderDayWeek <- factor(fraud$OrderDayWeek, levels= c("Sunday", "Monday", 
    "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"))
ggplot(fraud, aes(x = OrderDayWeek, fill = factor(Fraudulent), group = (Fraudulent) )) + geom_bar() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  scale_fill_manual(values=c('purple',
'forestgreen'))
```

```{r}
fraudulentDays <- fraud %>% group_by(OrderDayWeek, Fraudulent) %>% summarise(count = n()) %>% mutate(perc= round(count/sum(count)*100, digits = 2))
...
```{r}
fraudulentTimes <- fraud %>% group_by(OrderHour, Fraudulent) %>% summarise(count = n()) %>% mutate(perc= round(count/sum(count), digits = 3))
fraudulentTimes

ggplot(fraudulentTimes, aes(x = OrderHour, y = perc , fill = factor(Fraudulent), group = factor(Fraudulent))) + geom_bar(stat="identity") + theme(axis.text.x = element_text(angle = 90, hjust = 1))  + 
  scale_fill_manual(values=c('purple',
...
fraudulentTimesSites <- fraud %>% group_by(OrderHour, Website, Fraudulent) %>% summarise(count = n()) %>% mutate(perc= round(count/sum(count), digits = 3))
ggplot(fraudulentTimesSites, aes(x = OrderHour, y = perc , fill = factor(Fraudulent), group = factor(Fraudulent))) + geom_bar(stat="identity") + theme(axis.text.x = element_text(angle = 90, hjust = 1))  + 
  scale_fill_manual(values=c('purple',
